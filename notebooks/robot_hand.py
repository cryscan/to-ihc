...
class Visual(object):
    ...
    * the placement of the body with respect to the joint frame.
    ...
    def collision(self, c2, data=None, oMj1=None, oMj2=None):
        ...
        ab = pinv(np.atleast_2d(np.hstack([a1 - a2, b2 - b1]))) * (b2 - a2)
        ...
        if (0 <= ab).all() and (ab <= 1).all():
            ...
            if i == 0 or i == 2:  # fix a to pa, search b
                b = (pinv(np.atleast_2d(b1 - b2)) * (pa - b2))[0, 0]
                if b < 0:
                    pb = b2
                elif b > 1:
                    pb = b1
                ...
                a = (pinv(np.atleast_2d(a1 - a2)) * (pb - a2))[0, 0]
                if a < 0:
                    pa = a2
                elif a > 1:
                    pa = a1
        ...
        if norm(pa - pb) > 1e-3:
            ...
            r1 /= norm(r1)
            r2 = cross(ab, r1)
            R = np.hstack([r1, r2, np.atleast_2d(ab).T])
    ...
    def jacobian(self, c2, robot, q):
        Ja = pin.computeJointJacobian(robot.model, robot.data, q, self.jointParent)
        Jb = pin.computeJointJacobian(robot.model, robot.data, q, c2.jointParent)
